...
rmse = sqrt(sum((numeric_df[missing_matrix] - numeric_impute[missing_matrix])^2)/sum(missing_matrix))
rmse
```
...
plot(as.integer(df[missing_indicator, col]), as.integer(d1[missing_indicator, col]), xlab = 'actual values', ylab = 'imputed values', main = paste('Actual vs Imputed values: ', colnames(df)[col]))
```
